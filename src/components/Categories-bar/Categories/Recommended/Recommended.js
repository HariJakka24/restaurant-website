...
function Recommended({editItemObj, ...props}) {
    ...
    useEffect(() => {
        if(itemsList){
            if(isOn){
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("recommend").toLowerCase()) && item.itemType == "veg");
                setcategoryItems(filteredItems);
            }else{
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("recommend").toLowerCase()));
                setcategoryItems(filteredItems);
            }
        }
    }, [isOn, itemsList]);
        ...
        <div className="">
            ...
            {categoryItems && <div>
                    ...
                    {editItemObj != undefined &&
                        ...
                        <p className="text-danger">No Recommended available</p>
                    </div>
                    }
                </div> }
        </div>
