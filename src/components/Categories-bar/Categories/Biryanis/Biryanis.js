...
function Biryanis({editItemObj, ...props}) {
    ...
    useEffect(() => {
        if(itemsList){
            if(isOn){
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("biryani").toLowerCase()) && item.itemType == "veg");
                setcategoryItems(filteredItems);
            }else{
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("biryani").toLowerCase()));
                setcategoryItems(filteredItems);
            }
        }
    }, [isOn, itemsList]);
