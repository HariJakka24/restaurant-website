...
function Soups({editItemObj, ...props}) {
    ...
    useEffect(() => {
        if(itemsList){
            if(isOn){
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("soups").toLowerCase()) && item.itemType == "veg");
                setcategoryItems(filteredItems);
            }else{
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("soups").toLowerCase()));
                setcategoryItems(filteredItems);
            }
        }
    }, [isOn, itemsList]);
