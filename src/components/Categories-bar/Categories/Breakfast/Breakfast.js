...
function Breakfast({editItemObj, ...props}) {
    ...
    useEffect(() => {
        if(itemsList){
            if(isOn){
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("breakfast").toLowerCase()) && item.itemType == "veg");
                setcategoryItems(filteredItems);
            }else{
                let filteredItems = itemsList.filter(item => item.category.toLowerCase().includes(("breakfast").toLowerCase()));
                setcategoryItems(filteredItems);
            }
        }
    }, [isOn, itemsList]);
        ...
        <div className="">
            ...
            {categoryItems && <div>
                    ...
                    {editItemObj != undefined &&
                        ...
                        <p className="text-danger">No Soups available</p>
                    </div>
                    }
                </div> }
        </div>
